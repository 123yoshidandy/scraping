...
- **短時間に何度もアクセスすると遮断される。** 11件のAmazonターゲットを間を空けずにチェックしたところ、3件目以降の9件が約0.4秒で bot 判定のページを返し、`requireSelector` 不在の error になった。同一ホストへの最低アクセス間隔（後述）を入れると解消する
...
- ページの読み込み完了まで **約26秒** かかる。サーバーの応答自体が遅く（最初のバイトまで約20秒）、画像の読み込みを止めても短縮できない。既定のタイムアウト30秒では誤って error になりやすいため、同梱のターゲットは `timeoutMs: 60000` を指定している
...
- **同一ホストへの連続アクセスは最低5秒空ける**（`src/throttle.ts`）。相手サーバーへの配慮であり、bot 判定による遮断を避けるためでもある。ホストが異なれば待たない
- **ホストごとに直列・ホストをまたいで並列**にチェックする（同時に最大4ホスト、`MAX_CONCURRENT_HOSTS`）。同じサイトへ同時にアクセスすることはなく、待ち時間を別サイトのチェックで埋める
  - 1回の実行時間は「最も重いホスト1つ分」でほぼ決まる。ターゲットを増やすときは、同じサイトに偏らせないほうが速い
  - ターゲットごとに新しいブラウザコンテキストを作るため、同時に開くコンテキストは最大4つ
  - 実行ログは完了順に出るが、**state の反映とレポート表は `targets.json` の順序**で行うので、実行のたびに結果が入れ替わることはない
  - 同梱の `targets.json`（有効61件: ポケモンセンター34件 / Amazon 25件 / 楽天ブックス3件）で1回の実行は約175秒
